...
func handleCoolingModuleGCodeInsert(materialCoolingModulePercentage int) string {
	...
	if materialCoolingModulePercentage <= 0 {
		output += EOL + "; Cooling Module fan off" + EOL
		output += "M106 P10 S0" + EOL
	} else {
		output += EOL + "; Cooling Module fan %*255" + EOL
		output += "M106 P10 S" + strconv.Itoa(int((255*materialCoolingModulePercentage+50)/100)) + EOL
	}
	return output
}
